...
Fasta::Fasta(const std::string& infile) : size(_read(infile)),
sequences(_sequences), identifications(_identifications)
{
}
...
static void Fasta::write_to(std::ostream &os, InputIterator sequence_first, InputIterator sequence_last)
{
    using difference_type = decltype(std::distance(sequence_first, sequence_last));
    const difference_type number = std::distance(sequence_first, sequence_last);
    for (difference_type i = 0; i != number - 1; ++sequence_first, ++i)
        os << *sequence_first << std::endl;
    os << *sequence_first;
}

template<typename InputIterator1, typename InputIterator2>
static void Fasta::write_to(std::ostream &os, InputIterator1 sequence_first, InputIterator1 sequence_last,
    InputIterator2 identification_first)
{
    using difference_type = decltype(std::distance(sequence_first, sequence_last));
    const difference_type number = std::distance(sequence_first, sequence_last);
    for (difference_type i = 0; i != number - 1; ++sequence_first, ++identification_first, ++i)
    {
