#include "SuffixTreeBuilderTester.h"
#include "Fasta.h"
#include "Pseudo.h"
#include "Profile.h"
...
void suffixtree::SuffixTreeBuilderTester::test_profile(const char* lhs_file_path, const char* rhs_file_path)
{
    ...
    std::vector<std::string> lhs_sequences, rhs_sequences;
    lhs_sequences.reserve(lhs.sequences.size());
    rhs_sequences.reserve(rhs.sequences.size());
    pseudo::tranform_to_pseudo(lhs.sequences.cbegin(), lhs.sequences.cend(), std::back_inserter(lhs_sequences));
    pseudo::tranform_to_pseudo(rhs.sequences.cbegin(), rhs.sequences.cend(), std::back_inserter(rhs_sequences));
    ...
    // search
    auto result = profile_stb.search_for_prefix(rhs_profiles.cbegin() + 1, rhs_profiles.cend(), 2);
